...
#ifdef NGX_LOG_PATCH
        for (n = 1; n <= NGX_LOG_INTERNALLOG; n++) {
#else
...
#ifdef NGX_LOG_PATCH
    if (*head != new_log && new_log->log_level != NGX_LOG_EXPIREDFILE) {
        ngx_log_insert(*head, new_log);
    }

    if(new_log->log_level == NGX_LOG_EXPIREDFILE)
    {
    	cf->cycle->expired_file_log = new_log;
    }
#else
    if (*head != new_log) {
        ngx_log_insert(*head, new_log);
